...
public class Timer {
    ...
    public <T, U> double repeat(int n, Supplier<T> supplier, Function<T, U> function, UnaryOperator<T> preFunction, Consumer<U> postFunction) {
        ...
        for(int num = 0; num < n; num++) {
        	T resultAfterPre = null;
        	U afterFunction;
        	if(preFunction == null) {
        		resultAfterPre = supplier.get();
        	}
        	else if (preFunction != null){
        		resultAfterPre = preFunction.apply(supplier.get());
        	}
        	
        	resume();
        	afterFunction = function.apply(resultAfterPre);
        	pauseAndLap();
        	
        	if (postFunction != null) {
				postFunction.accept(afterFunction);
			}
        }
